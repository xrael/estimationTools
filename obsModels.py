...
class rangeRangeRateObsModel(observerModelBase):
    ...
    def __init__(self, stateSymb, params, observerCoordinates, inputSymb = None):
        super(rangeRangeRateObsModel, self).__init__("RangeRangeRate", stateSymb, params, observerCoordinates, inputSymb)
    ...
    def getObserverModel(cls, theta_0, angular_rate, R_eq, e_planet, GS_coordinates):
        ...
        inputSymb = rangeRangeRateObsModel.buildSymbolicInput()
        obs_mod = rangeRangeRateObsModel(symbState, params, GS_coordinates, inputSymb)
    ...
    def computeInputJacobian(self, X, t, params, u = None):
        """
        Computes the Jacobian wrt the input of G (Htilde).
        :param X: State.
        :param t: Time.
        :param params: Parameters of the model.
        :return: The jacobian evaluated at (X,t).
        ...
        Htilde_inputs = np.zeros([nmbrOfOutputs,nmbrOfInputs])
        ...
        for i in range(0,nmbrOfOutputs):
            for j in range(0, nmbrOfInputs) :
                Htilde_inputs[i][j] = self._jacobianInputLambda[i][j](x, y, z, x_dot, y_dot, z_dot, x_gs, y_gs, z_gs, theta, theta_dot)

        return Htilde_inputs
    ...
    def computeSymbolicJacobian(self):
        self.getObsModelDerivativesFromPosVel()
        self.getObsModelDerivativesFromGSpos()
    ...
    def getObsModelDerivativesFromPosVel(self):
        G = self._modelSymb
    ...
    def getObsModelDerivativesFromGSpos(self):
        G = self._modelSymb
    ...
    def computeSymbolicInputJacobian(self):
        ...
        dG = [[0 for i in range(0,nmbrOfInputs)] for i in range(0,nmbrOfOutputs)]
        Htilde_input_lambda = [[0 for i in range(0,nmbrOfInputs)] for i in range(0,nmbrOfOutputs)]

        for i in range(0, nmbrOfOutputs) :
            for j in range(0, nmbrOfInputs) :
                dG[i][j] = sp.diff(G[i], self._inputSymb[j])
                #print "Jacobian[",i,",",j,"]: ", dG[i][j]
                Htilde_input_lambda[i][j] = sp.lambdify((x, y, z, x_dot, y_dot, z_dot, x_gs, y_gs, z_gs, theta, theta_dot), dG[i][j], "numpy")

        self._jacobianInputSymb = dG
        self._jacobianInputLambda = Htilde_input_lambda
...
class rightAscensionDeclinationObsModel(observerModelBase):
    ...
    def __init__(self, stateSymb, params, observerCoordinates, inputSymb = None):
        super(rightAscensionDeclinationObsModel, self).__init__("RightAscensionDeclination", stateSymb, params, observerCoordinates, inputSymb)
    ...
    def getObserverModel(cls, theta_0, angular_rate, R_eq, e_planet, GS_coordinates):
        ...
        inputSymb = rangeRangeRateObsModel.buildSymbolicInput()
        obs_mod = rightAscensionDeclinationObsModel(symbState, params, GS_coordinates, inputSymb)
